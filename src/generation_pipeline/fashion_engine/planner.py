    ...
    def evaluate_outfits(self, combinations: List[Tuple], style_profiles: Dict[str, torch.Tensor], 
        ...
        for i, combo in enumerate(combinations):
            ...
            results.append({
                ...
                "shirt": combo[1]['path'].split('/')[-1],
                "bottom": combo[0]['path'].split('/')[-1],
                "top": combo[1]['path'].split('/')[-1],
            })
...
# ==================== [원래 코드 - outer 사용] ====================
# def generate_combinations(self, recommendations, top_n=3, include_outer=True):
#     outers = recommendations.get("outer", [])[:top_n] if include_outer else [None]
#     # 3개 조합: itertools.product(pants, outers, shirts)
# 
# def evaluate_outfits(self, ...):
#     # 3장 평균 임베딩
